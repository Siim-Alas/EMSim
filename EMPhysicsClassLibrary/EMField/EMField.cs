...
namespace EMPhysicsClassLibrary.EMField
{
    public class EMField
    {
        ...
        /// <returns></returns>
        private static double GetRetardedElectricPotentialFromStationaryPointCharge(
            ...
            double t)
        {
            ...
            // Note that the point charge is assumed to behave like the Dirac delta function 
            // (https://en.wikipedia.org/wiki/Dirac_delta_function), generalized in the Wikipedia 
            // article to vectors via the definition: integral over R^n of f(x)delta(dx) = f(0), 
            // where f(x): R^n -> R^m, 0 is the 0-vector, and delta(dx) is the dirac delta function.
            // This definition allows us to rewrite the retarded electric potential as 
            // (1 / 4 pi epsilon0) * (rho(r', tR) / |r - r'|).
